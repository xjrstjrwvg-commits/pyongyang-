...
def search():
    ...
    if end_char and ignore_voiced: end_char = end_char.translate(VOICE_MAP)
    ...
    def solve(path):
        ...
        if len(path) == max_len:
            ...
            if end_char and get_clean_char(path[-1], ignore_voiced, "tail") != end_char: return
            if must_chars:
                full_text = "".join(path).translate(VOICE_MAP if ignore_voiced else {})
                if not any(mc in full_text for mc in must_chars): return
        ...
        try:
            ...
            for next_w in pool:
                if next_w not in path:
                    ...
                    if next_c == target: solve(path + [next_w])
        except: return
